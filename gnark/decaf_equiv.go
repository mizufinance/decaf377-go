...
func AssertEquivalent(api frontend.API, left, right gnarkte.Point) {
	api.AssertIsEqual(api.Mul(left.X, right.Y), api.Mul(right.X, left.Y))
}

func AssertEquivalentIf(api frontend.API, left, right gnarkte.Point, cond frontend.Variable) {
...
func EquivalentNative(left, right gnarkte.Point) bool {
	modulus := ScalarField()
