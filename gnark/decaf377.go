...
const FieldBits = decaf377.FieldBits
...
func decaf377Abs(api frontend.API, value frontend.Variable) frontend.Variable {
	bits := api.ToBinary(value, FieldBits)
...
// CompressToField implements the minimal Penumbra decaf377 quotient gadget
// needed by spend: quotient-group compression to a single field element.
func CompressToField(api frontend.API, point gnarkte.Point) (frontend.Variable, error) {
...
func EncodeToCurve(api frontend.API, r0 frontend.Variable) (gnarkte.Point, error) {
	curve, err := gnarkte.NewEdCurve(api, curves.BLS12_377)
	if err != nil {
	...
	isNegative := api.ToBinary(s, FieldBits)[0]
...
func EncodeToCurveNative(input *big.Int) (gnarkte.Point, error) {
	constants, err := loadDecaf377Constants()
	if err != nil {
		return gnarkte.Point{}, err
	}
...
func CompressToFieldNative(point gnarkte.Point) (*big.Int, error) {
	constants, err := loadDecaf377Constants()
	if err != nil {
		return nil, err
	}
